...
class _MessageCoachScreenState extends State<MessageCoachScreen> {
            ...
            BlocConsumer<MessageBloc, MessageState>(
              listener: (context, state) {
                if (state.status == MessageStatus.addSuccess) {
                  _showSnackBar(context, 'Message ajouté', Colors.green, Icons.check_circle);
                  _messageController.text = "";
                } else if (state.status == MessageStatus.error) {
                  _showSnackBar(context, state.error, Colors.yellowAccent, Icons.error);
                }
              },
              builder: (context, state) {
                switch (state.status) {
  ...
  void _onAddMessage(BuildContext context) async {
    ...
    if (_messageController.text != "") {
      ...
      _showSnackBar(context, "Ecrivez un message", Colors.redAccent, Icons.cancel);
    }
  }
