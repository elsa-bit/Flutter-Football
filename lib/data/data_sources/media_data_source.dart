...
class MediaDataSource extends BaseDataSource with MediaService {
  ...
  Future<String> getAvatar(String name) async {
    return await supabase.storage.from('avatars').createSignedUrl(name, 7200);
  }
  ...
  Future<String> getClubRule() async {
    return await supabase.storage
        .from('rule')
        .createSignedUrl('club_rule.pdf', 7200);
  }

  Future<String> getCoachRule() async {
    return await supabase.storage
        .from('rule')
        .createSignedUrl('coach_rule.pdf', 7200);
  }

  Future<String> getDocumentClub() async {
    return await supabase.storage
        .from('rule')
        .createSignedUrl('doc_club.pdf', 7200);
  }
  ...
  Future<List<String>> getMatchBucketImages(String matchId) async {
    final result =
        await supabase.storage.from("match-resources").list(path: matchId);
    ...
    for (var file in result) {
      try {
        String url = await supabase.storage
            ...
            .createSignedUrl(
                "$matchId/${file.name}", 7200); // 7200 secondes = 2 hours
        imageUrls.add(url);
      } catch (e) {
  ...
  Future<String> getSpecificVideos(String idplayer) async {
    ...
    final response =
        await httpGet(Endpoints.specificVideosPlayerPath, queryParameters);
  ...
  Future<String> updateProfilePicture(File imageFile, String filename) async {
    String result = await predict(imageFile.path);
    debugPrint(result);

    if (result != "Control") {
      throw Exception("Votre image contient du contenu $result !");
    }
